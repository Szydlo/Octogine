...
using Octo::SkeletalModel;

SkeletalModel::SkeletalModel(std::string path, bool useColor, glm::vec3 color) // THIS CODE HAS MEMORY LEAK @TODO FIX IT SOMEDAY
...
void SkeletalModel::initMesh(aiNode* node, const aiScene* scene)
{
...
void SkeletalModel::setVertexBoneData(SVertex& vertex, int boneID, float weight)
{
	for (int i = 0; i < scMaxBoneInfluence; i++)
	{
...
void SkeletalModel::setVertexBoneDataToDefault(SVertex& vertex)
{
	for (int i = 0; i < scMaxBoneInfluence; i++)
	{
...
void SkeletalModel::extractBoneWeightForVertices(std::vector<SVertex>& vertices, aiMesh* mesh, const aiScene* scene)
{
	...
	for (int boneIndex = 0; boneIndex < mesh->mNumBones; boneIndex++)
	{
		...
		if (boneInfoMap.find(boneName) == boneInfoMap.end())
		{
			...
			newBoneInfo.offset = sConvertMatrixToGLMFormat(mesh->mBones[boneIndex]->mOffsetMatrix);
